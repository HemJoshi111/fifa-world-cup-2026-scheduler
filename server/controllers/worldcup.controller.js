...
async function fetchNationalFlag(teamName) {
    const lookupKey = normalizeCountryLookupKey(teamName);
    if (!lookupKey) return '';
...
async function enrichFixturesWithNationalFlags(fixtures) {
    ...
    fixtures.forEach((match) => {
        if (match?.home?.name) uniqueTeamNames.add(match.home.name);
        if (match?.away?.name) uniqueTeamNames.add(match.away.name);
    });

    await Promise.all(Array.from(uniqueTeamNames).map((teamName) => fetchNationalFlag(teamName)));

    return fixtures.map((match) => {
        const homeFlag = match?.home?.name ? nationalFlagCache.get(normalizeCountryLookupKey(match.home.name)) || '' : '';
        const awayFlag = match?.away?.name ? nationalFlagCache.get(normalizeCountryLookupKey(match.away.name)) || '' : '';
...
const getFixtures = async (req, res) => {
    try {
        if (req.query.page) {
            const response = await apiClient.get('/fixtures', { params: req.query });
            const payload = response.data;
            const fixtures = unwrapFixturesResponse(payload);
            const enrichedFixtures = await enrichFixturesWithNationalFlags(fixtures);

            if (Array.isArray(payload)) {
                return res.status(200).json(enrichedFixtures);
            }

            if (Array.isArray(payload?.data)) {
                return res.status(200).json({ ...payload, data: enrichedFixtures });
            }
        ...
        while (page <= 10) {
            const fixtures = await fetchFixturesPage(req.query, page);
        ...
        const enrichedFixtures = await enrichFixturesWithNationalFlags(allFixtures);
